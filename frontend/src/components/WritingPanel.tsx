...
export default function WritingPanel({ open, onClose, onGenerate, model }: WritingPanelProps) {
  ...
  const handleGenerate = () => {
    ...
    const signText = sign && date ? `${sign} · ${date}` : (sign || date || '')
    const prompt = `请帮我撰写一份${docType}。标题：${title || '未指定'}；正文要点：${body}`
    const writing: WritingRequest = { docType, title, to, body, sign: signText }
    setGenerating(true)
    ...
    <>
      <div className={`doc-panel ${open ? 'open' : ''}`}>
        <div className="doc-head">
          <h3><Icon name="pen-line" size={18} /> 公文写作助手</h3>
          <button className="doc-close" onClick={onClose} aria-label="关闭">
            <Icon name="x" size={18} />
          </button>
        </div>
        ...
        <div className="doc-form">
          ...
          <div className="doc-field">
            ...
            <textarea value={body} onChange={(e) => setBody(e.target.value)} placeholder="输入会议时间、地点、参会人员、会议内容等要点…" rows={5} />
          </div>
          <div className="doc-field-row">
            <div className="doc-field">
              <label>落款单位</label>
              <input value={sign} onChange={(e) => setSign(e.target.value)} placeholder="如：办公室" />
            </div>
            <div className="doc-field">
              <label>日期</label>
